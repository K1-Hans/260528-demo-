...
import {
  ChevronDown, ChevronRight, Search, Bell, Sun, Moon, LogOut, Repeat, AlertTriangle, ShieldHalf, Menu, Lock,
} from 'lucide-react';
...
function LiveClock() {
  ...
  return (
    <div className="live-cluster" title="实时风险引擎在线 · 毫秒级决策">
    ...
    <div className={`app-shell ${mobileNav ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar">
        <button className="sidebar-logo" onClick={() => navigate(firstAllowedPath(hasPermission))}>
          <div className="logo-mark"><ShieldHalf size={19} /></div>
          <div style={{ textAlign: 'left' }}>
            <div className="logo-title">风控作战室</div>
            <div className="logo-sub">实时决策 · 可解释合规</div>
          </div>
        </button>
      ...
      <div className="main">
        <header className="topbar">
          ...
          {/* 命令台式全局搜索（按卡号/案件号/客户号/规则名） */}
          <button className="global-search" onClick={() => setPalette(true)}>
            <Search size={15} />
            <span className="global-search-ph">按卡号 / 案件号 / 客户号 / 规则名检索 · 或执行快捷处置</span>
          ...
          <div className="row gap-2" style={{ marginLeft: 'auto', position: 'relative' }}>
            ...
            <button className="icon-btn" onClick={toggle} title={`主题：${mode === 'dark' ? '深色作战' : '浅色报表'} · 点击切换`}>
              {mode === 'dark' ? <Moon size={17} /> : <Sun size={17} />}
            </button>

            <div style={{ position: 'relative' }}>
              <button className="icon-btn" onClick={() => setMenu(m => m === 'notif' ? 'none' : 'notif')} title="实时告警">
                <Bell size={17} />{dangerCount > 0 && <span className="dot-badge" />}
              </button>
              {menu === 'notif' && (
                <Dropdown onClose={() => setMenu('none')}>
                  <div className="label" style={{ padding: '4px 8px 10px' }}>实时告警 · {ALERTS.length}</div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {menu === 'user' && (
                <Dropdown onClose={() => setMenu('none')} width={282}>
        ...
        <div className="content">
          <div className="content-anim" key={location.pathname}>
            {denied
              ? <div className="page"><EmptyState icon={<Lock size={36} />} title="无访问权限" desc="当前角色无权访问此模块。权限即职责边界 — 风控分权确保合规。如需开通请联系平台管理员。" /></div>
              : <Outlet />}
          </div>
        </div>
      </div>
