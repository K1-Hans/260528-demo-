...
// 织流 Agent 编排：accent = 电光青（--gold token）· run-state ramp 待运行灰/执行中青/完成绿/出错红/等人审黄 · 序列 --c1..c8。
...
/** run-state 色：state → 解析后色值（canvas 安全）。待运行灰/执行中青/完成绿/出错红/等人审黄。 */
export function runStateColor(s: 'idle' | 'running' | 'done' | 'error' | 'wait'): string {
  return cssVar({ idle: '--text-3', running: '--gold', done: '--success', error: '--danger', wait: '--warning' }[s]);
}
...
export const SEMANTIC = {
  active: '--gold', done: '--emerald', warn: '--warning', error: '--danger', info: '--info',
} as const;
...
export function baseOption(): Record<string, unknown> {
  ...
  return {
    ...
    tooltip: {
      ...
      extraCssText: 'border-radius:11px;box-shadow:0 10px 34px rgba(0,0,0,.40);',
