    ...
    <div className={`app-shell ${mobileNav ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar">
        <button className="sidebar-logo" onClick={() => navigate(firstAllowedPath(hasPermission))}>
          ...
          <div style={{ textAlign: 'left' }}>
            <div className="logo-title">智库 · 企业知识中台</div>
            <div className="logo-sub">权限感知 · 可溯源</div>
          </div>
        </button>
        ...
        <div className="sidebar-footer">
          ...
          <div className="flex-1">
            ...
            <div className="uf-role" style={{ color: currentRole?.color }}>{currentRole?.name} · 密级 {currentRole?.clearance}</div>
          </div>
      ...
      <div className="main">
        <header className="topbar">
          ...
          {/* Glean 式醒目全局搜索 */}
          <button className="global-search" onClick={() => { navigate('/'); setTimeout(() => document.getElementById('kb-search-input')?.focus(), 60); }}>
            <Search size={16} />
            <span className="global-search-ph">搜索全公司知识 · 语义 + 关键词 · 按你的权限过滤</span>
            <span className="kbd">⌘K</span>
          </button>

          <div className="row gap-2" style={{ marginLeft: 'auto', position: 'relative' }}>
            <div className="row gap-1 svc-pill" title="每次检索按你的权限严格过滤 · 每个答案可溯源回原文">
              <ShieldCheck size={13} /><span>权限感知 · 可溯源</span>
            </div>
                  ...
                  {MOCK_USERS.filter(u => u.status === 'active').map(u => {
                    ...
                    return (
                      <div key={u.id} className={`row gap-2 cmdk-item ${u.id === user?.id ? 'cmdk-item-active' : ''}`} style={{ padding: '8px' }} onClick={() => { switchUser(u.id); setMenu('none'); navigate('/'); }}>
                        ...
                        <span style={{ fontSize: 11, marginLeft: 'auto', color: role?.color, fontWeight: 600 }}>{role?.name} · 密级 {role?.clearance}</span>
                      </div>
                    );
                  })}
