...
export function StatusBadge({ status, tone = 'muted', dot = true }: { status: string; tone?: 'good' | 'warn' | 'bad' | 'info' | 'cost' | 'muted'; dot?: boolean }) {
  const c = { good: 'var(--success)', warn: 'var(--warning)', bad: 'var(--danger)', info: 'var(--gold)', cost: 'var(--cost)', muted: 'var(--text-3)' }[tone];
  return <span className="badge" style={{ background: `color-mix(in srgb, ${c} 14%, transparent)`, color: c }}>
    {dot && <span style={{ width: 5, height: 5, borderRadius: '50%', background: c, display: 'inline-block' }} />}{status}
  </span>;
}
