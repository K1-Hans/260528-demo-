...
import { NAV, FLAT_ROUTES, crumbFor, type NavGroup } from '../lib/nav';
...
export default function Layout() {
  ...
  // 按角色权限过滤导航（spec 01：角色切换实时改导航可见项）
    ...
    <div className={`app-shell ${mobileNav ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar">
        <div className="sidebar-logo">
          <div className="logo-mark">智</div>
          <div>
            <div className="logo-title">智能客服运营中台</div>
            <div className="logo-sub">智能客服运营 · v2</div>
          </div>
        </div>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="row gap-2" style={{ marginLeft: 'auto', position: 'relative' }}>
            <div className="row gap-1 svc-pill" title="检索引擎就绪 · 语义模式">
              <ShieldCheck size={13} /><span>合规审计 · 100% 留痕</span>
            </div>
            ...
            <div style={{ position: 'relative' }}>
              <button className="icon-btn" onClick={() => setMenu(m => m === 'notif' ? 'none' : 'notif')} title="运营预警">
                <Bell size={17} />{dangerCount > 0 && <span className="dot-badge" />}
              </button>
              {menu === 'notif' && (
                <Dropdown onClose={() => setMenu('none')}>
                  <div className="label" style={{ padding: '4px 8px 10px' }}>运营预警 · {ALERTS.length}</div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {menu === 'user' && (
                <Dropdown onClose={() => setMenu('none')} width={248}>
                  <div className="label" style={{ padding: '4px 8px 8px' }}>切换演示账号（体验 RBAC 分权）</div>
                  {MOCK_USERS.filter(u => u.status === 'active').map(u => {
                    const role = ROLES.find(r => r.id === u.role);
                    return (
                      <div key={u.id} className={`row gap-2 cmdk-item ${u.id === user?.id ? 'cmdk-item-active' : ''}`} style={{ padding: '8px' }} onClick={() => { switchUser(u.id); setMenu('none'); }}>
        ...
        <div className="content">
          <div className="content-anim" key={location.pathname}>
            {denied
              ? <div className="page"><EmptyState icon={<Lock size={36} />} title="无访问权限" desc="当前角色无权访问此模块，如需开通请联系运营总监" /></div>
              : <Outlet />}
          </div>
        </div>
      </div>
