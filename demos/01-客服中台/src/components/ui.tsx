...
// ─── TrendChip ───────────────────────────────────────────────────────────────
export function TrendChip({ change, suffix = '环比' }: { change: number; suffix?: string }) {
  const up = change >= 0;
  const Icon = change === 0 ? Minus : up ? TrendingUp : TrendingDown;
  return (
    <span className={`row gap-1 tnum ${up ? 'trend-up' : 'trend-down'}`} style={{ fontSize: 12, fontWeight: 600 }}>
      <Icon size={12} />
      {up ? '+' : ''}{change}%
...
// ─── StatCard ────────────────────────────────────────────────────────────────
export function StatCard({ label, raw, unit, change, spark, decimals, icon, delayClass = '' }: {
  label: string; raw: number; unit?: string; change?: number; spark?: number[];
  decimals?: number; icon?: React.ReactNode; delayClass?: string;
}) {
    ...
    <div ref={ref} className={`card card-hover reveal ${delayClass}`}>
      ...
      <div className="row spread" style={{ alignItems: 'flex-end' }}>
        <div>
          ...
          {change !== undefined && <div style={{ marginTop: 8 }}><TrendChip change={change} /></div>}
        </div>
        {spark && <Sparkline data={spark} color={change !== undefined && change < 0 ? 'var(--danger)' : 'var(--emerald)'} />}
      </div>
    </div>
  );
}
