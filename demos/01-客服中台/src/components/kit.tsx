...
import type { KBStatus, RiskType, BadcaseStatus, RejectStatus } from '../types';

// ─── 状态徽章 ────────────────────────────────────────────────────────────────
const KB_TONE: Record<KBStatus, string> = { 已生效: 'var(--success)', 待发布: 'var(--warning)', 已下线: 'var(--text-3)' };
export function StatusBadge({ status }: { status: KBStatus }) {
  const c = KB_TONE[status];
...
const RISK_TONE: Record<RiskType, string> = {
  投诉维权: 'var(--warning)', 法律维权: 'var(--danger)', 金融监管: 'var(--danger)', 催收相关: 'var(--warning)',
  媒体曝光: 'var(--info)', 涉政敏感: 'var(--danger)', 合规风险: 'var(--warning)', 扬言轻生: 'var(--danger)',
};
export function RiskBadge({ type }: { type: RiskType }) {
  const c = RISK_TONE[type];
  return <span className="badge" style={{ background: `color-mix(in srgb, ${c} 14%, transparent)`, color: c }}>{type}</span>;
}

const BC_TONE: Record<BadcaseStatus, [string, string]> = { pending: ['var(--danger)', '待处理'], fixed: ['var(--success)', '已修复'], ignored: ['var(--text-3)', '已忽略'] };
export function BadcaseBadge({ status }: { status: BadcaseStatus }) {
  const [c, t] = BC_TONE[status];
  return <span className="badge" style={{ background: `color-mix(in srgb, ${c} 14%, transparent)`, color: c }}>{t}</span>;
}

const RJ_TONE: Record<RejectStatus, [string, string]> = { pending: ['var(--warning)', '待处理'], added_qa: ['var(--success)', '已新增 QA'], ignored: ['var(--text-3)', '已忽略'] };
export function RejectBadge({ status }: { status: RejectStatus }) {
  const [c, t] = RJ_TONE[status];
  return <span className="badge" style={{ background: `color-mix(in srgb, ${c} 14%, transparent)`, color: c }}>{t}</span>;
}
