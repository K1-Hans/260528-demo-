...
import {
  ChevronDown, ChevronRight, Search, Bell, Sun, Moon, LogOut, Repeat, AlertTriangle, Headset, Menu, Lock,
} from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { useTheme } from '../contexts/ThemeContext';
import { NAV, FLAT_ROUTES, crumbFor, firstAllowedPath, type NavGroup } from '../lib/nav';
import { ALERTS, MOCK_USERS, ROLES, COMPLIANCE_LIGHTS } from '../lib/mockData';
import { EmptyState } from './ui';
import { ComplianceLights } from './sig';
import CommandPalette from './CommandPalette';
import './Layout.css';

/** 顶栏 LIVE 时钟（实时话务感 · 等宽跳秒）。 */
function LiveClock() {
  ...
  return (
    <div className="live-cluster" title="外呼引擎实时在线">
    ...
    <div className={`app-shell ${mobileNav ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar">
        <button className="sidebar-logo" onClick={() => navigate(firstAllowedPath(hasPermission))}>
          <div className="logo-mark"><Headset size={18} /></div>
          <div style={{ textAlign: 'left' }}>
            <div className="logo-title">话务作战室</div>
            <div className="logo-sub">实时外呼 · 合规优先</div>
          </div>
        </button>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <button className="global-search" onClick={() => setPalette(true)}>
            <Search size={15} />
            <span className="global-search-ph">按活动 / 客户 / 话术 / 录音检索 · 或执行快捷操作</span>
            <span className="kbd">⌘K</span>
          </button>

          <div className="row gap-2" style={{ marginLeft: 'auto', position: 'relative' }}>
            {/* 合规三灯（规格 §④ 顶栏常驻） */}
            <div className="comp-lights-wrap"><ComplianceLights lights={COMPLIANCE_LIGHTS} /></div>
            <LiveClock />

            <button className="icon-btn" onClick={toggle} title={`主题：${mode === 'dark' ? '深色话务' : '浅色质检'} · 点击切换`}>
              {mode === 'dark' ? <Moon size={17} /> : <Sun size={17} />}
            </button>

            <div style={{ position: 'relative' }}>
              <button className="icon-btn" onClick={() => setMenu(m => m === 'notif' ? 'none' : 'notif')} title="合规告警">
                <Bell size={17} />{dangerCount > 0 && <span className="dot-badge" />}
              </button>
              {menu === 'notif' && (
                <Dropdown onClose={() => setMenu('none')}>
                  <div className="label" style={{ padding: '4px 8px 10px' }}>合规告警 · {ALERTS.length}</div>
        ...
        <div className="content">
          <div className="content-anim" key={location.pathname}>
            {denied
              ? <div className="page"><EmptyState icon={<Lock size={36} />} title="无访问权限" desc="当前角色无权访问此模块。权限即职责边界 — 外呼分权确保合规。如需开通请联系平台管理员。" /></div>
              : <Outlet />}
          </div>
        </div>
      </div>
