...
import { Search, CornerDownLeft, Moon, Sun, LogOut, ShieldAlert, CheckCheck, Scale, Fingerprint } from 'lucide-react';
...
export default function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const items = useMemo(() => {
    ...
    // 审核快捷操作（命令台即审核指令）
    const quick = [
      hasPermission('queue:act') && { type: 'quick' as const, label: '批量下架当前队列高危项', sub: '快捷操作', icon: <ShieldAlert size={15} />, run: () => toast('已下架 3 件高危内容 · 处置留痕已记录', 'danger') },
      hasPermission('queue:act') && { type: 'quick' as const, label: '一键通过安全项', sub: '快捷操作', icon: <CheckCheck size={15} />, run: () => toast('已通过 12 件低风险内容', 'success') },
      hasPermission('appeal:read') && { type: 'quick' as const, label: '查看待复核申诉', sub: '快捷查询', icon: <Scale size={15} />, run: () => navigate('/appeal') },
      hasPermission('synthetic:read') && { type: 'quick' as const, label: '查最新合成内容命中', sub: '快捷查询', icon: <Fingerprint size={15} />, run: () => navigate('/synthetic') },
    ].filter(Boolean) as { type: 'quick'; label: string; sub: string; icon: React.ReactNode; run: () => void }[];
    const actions = [
      { type: 'action' as const, label: mode === 'dark' ? '切换到浅色审核主题' : '切换到深色取证主题', sub: '外观', icon: mode === 'dark' ? <Sun size={15} /> : <Moon size={15} />, run: toggle },
    ...
    <div className="cmdk-overlay" onClick={onClose}>
      <div className="cmdk-panel" onClick={e => e.stopPropagation()}>
        <div className="cmdk-input-row">
          ...
          <input className="cmdk-input" placeholder="检索内容 / 工单 / 策略 / 发布者 · 执行审核命令…" value={q} autoFocus onChange={e => setQ(e.target.value)} />
