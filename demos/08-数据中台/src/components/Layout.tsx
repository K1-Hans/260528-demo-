    ...
    <div className={`app-shell ${mobileNav ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar">
        <button className="sidebar-logo" onClick={() => navigate(firstAllowedPath(hasPermission))}>
          <div className="logo-mark">演</div>
          <div style={{ textAlign: 'left' }}>
            <div className="logo-title">演算台 · 数据中台</div>
            <div className="logo-sub">NL 问数 · 可信治理</div>
          </div>
        </button>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <button className="global-search" onClick={() => setPalette(true)}>
            <Search size={16} />
            <span className="global-search-ph">问数 / 指标 / 字段 / 血缘检索 · 或执行快捷操作</span>
            <span className="kbd">⌘K</span>
          </button>

          <div className="row gap-2" style={{ marginLeft: 'auto', position: 'relative' }}>
            <div className="row gap-1 svc-pill" title="查询走治理语义层 · 答不了报错，绝不返回错数">
              <ShieldCheck size={13} /><span>可信治理 · 答不了报错</span>
            </div>
