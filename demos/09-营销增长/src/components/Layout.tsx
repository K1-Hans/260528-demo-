    ...
    <div className={`app-shell ${mobileNav ? 'nav-open' : ''}`}>
      ...
      <aside className="sidebar">
        <button className="sidebar-logo" onClick={() => navigate(firstAllowedPath(hasPermission))}>
          <div className="logo-mark">增</div>
          <div style={{ textAlign: 'left' }}>
            <div className="logo-title">增势 · 营销增长</div>
            <div className="logo-sub">洞察 · 创意 · 投放 · 归因</div>
          </div>
        </button>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <button className="global-search" onClick={() => setPalette(true)}>
            <Search size={16} />
            <span className="global-search-ph">搜活动 / 人群 / 创意 / 渠道 · 或执行快捷操作</span>
            <span className="kbd">⌘K</span>
          </button>

          <div className="row gap-2" style={{ marginLeft: 'auto', position: 'relative' }}>
            <div className="row gap-1 svc-pill" title="Agent 提效全链路 · 关键决策保留人审卡点（合规/预算/授权）">
              <ShieldCheck size={13} /><span>Agent 驱动 · 关键卡点人审</span>
            </div>
